...
import { getContract, loadAbi } from 'src/utils/abi-loader';
  ...
  async deployContract(
    ...
    if (Array.isArray(artifact.abi)) {
      throw new BadRequestException(
        'Stored ABI is an ABI array without bytecode. For deploy, upload the full artifact (including bytecode) to DynamoDB.'
      );
    }
    ...
    let abi: any = artifact.abi.abi ?? artifact; // fallback
    let bytecode: string | undefined = artifact.abi.bytecode ?? (artifact.abi.evm && artifact.abi.evm.bytecode && artifact.abi.evm.bytecode.object);
    ...
    try {
      ...
      const unsignedTx: any = (factory as any).getDeployTransaction(...deployArgs);
